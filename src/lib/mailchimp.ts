...
export async function subscribeToNewsletter(
  ...
  if (!listId) {
    console.error(
      "🚨 MAILCHIMP_AUDIENCE_ID environment variable not configured",
    );
    return {
      success: false,
      message:
  ...
  try {
    ...
    const memberData = {
      ...
      tags: ["bali-report", "tropical-news", ...(data.interests || [])],
    };
    ...
    const response = (await mailchimp.lists.addListMember(
      listId,
      memberData,
    )) as unknown as MailchimpMemberResponse;
    ...
    return {
      success: true,
      message:
        "Welcome to Bali Report! 🏝️ You'll receive tropical news updates in your inbox.",
 ...
 */
export async function validateMailchimpConfig(): Promise<{
  isValid: boolean;
  missing: string[];
}> {
  const required = [
