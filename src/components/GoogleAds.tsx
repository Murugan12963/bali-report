...
const GoogleAds: React.FC<GoogleAdsProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'production' && adSlot) {
      ...
      // Development logging
      console.log(`Google AdSense ${type} ad would load here with slot ID: ${adSlot || 'NOT_SET'}`);
      setIsLoaded(true);
    }
  ...
  if (hasError) {
    return (
      <div className={`bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg flex flex-col items-center justify-center text-red-600 dark:text-red-400 text-sm p-4 theme-transition ${className}`}>
        <div className="text-center">
          <div className="font-medium mb-1">⚠️ Ad Load Error</div>
          <div className="text-xs text-red-500 dark:text-red-400">Google AdSense failed to load</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div 
        ...
        style={{ 
          minHeight: type === 'banner' || type === 'leaderboard' ? '90px' : 
                    type === 'responsive' ? '100px' : 
                    type === 'native' ? '120px' : '250px',
      ...
      >
        <div className="text-center p-4 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 rounded transition-colors theme-transition">
          ...
          <div className="text-xs text-gray-400 dark:text-gray-500">{adConfig.dimensions}</div>
          <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">
            Slot: {effectiveAdSlot ? effectiveAdSlot.slice(0, 10) + '...' : 'Not Set'}
          </div>
          <div className="text-xs text-blue-600 dark:text-blue-400 mt-1">✅ Google AdSense Ready</div>
        </div>
      </div>
    );
  }
