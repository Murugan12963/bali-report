...
const ThemeSwitcher: React.FC<ThemeSwitcherProps> = ({
  ...
  if (!mounted) {
    return (
      <div
        className={`w-10 h-10 bg-gray-200 rounded-lg animate-pulse ${className}`}
      />
    );
  }
    ...
    return (
      ...
      >
        <div className="relative w-5 h-5">
          ...
          `}
          >
            <div className="text-amber-500">{getThemeIcon("light")}</div>
          </div>
          ...
          `}
          >
            <div className="text-blue-400">{getThemeIcon("dark")}</div>
          </div>
        </div>
